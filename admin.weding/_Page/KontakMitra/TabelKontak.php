<?php
    ...
    if(empty($SessionIdAkses)){
        ...
        echo '      <small>Sessi Akses Sudah Berakhir, Silahkan Login Ulang</small>';
        echo '  </div>';
        echo '</div>';
    }else{
        //Keyword_by
        ...
        if(empty($keyword_by)){
            if(empty($keyword)){
                $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM kontak WHERE id_mitra='$SessionIdAkses'"));
            }else{
                $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM kontak WHERE (id_mitra='$SessionIdAkses') AND (id_akses like '%$keyword%' OR nama like '%$keyword%' OR email like '%$keyword%' OR kontak like '%$keyword%' OR sumber like '%$keyword%'  OR sudah_dihubungi like '%$keyword%')"));
            }
        }else{
            if(empty($keyword)){
                $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM kontak WHERE id_mitra='$SessionIdAkses'"));
            }else{
                $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM kontak WHERE (id_mitra='$SessionIdAkses') AND ($keyword_by like '%$keyword%')"));
            }
        }
        //Mengatur Halaman
                ...
                <tbody>
                    <?php
                        if(empty($jml_data)){
                            ...
                            if(empty($keyword_by)){
                                if(empty($keyword)){
                                    $query = mysqli_query($Conn, "SELECT*FROM kontak WHERE id_mitra='$SessionIdAkses' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                                }else{
                                    $query = mysqli_query($Conn, "SELECT*FROM kontak WHERE (id_mitra='$SessionIdAkses') AND (id_akses like '%$keyword%' OR nama like '%$keyword%' OR email like '%$keyword%' OR kontak like '%$keyword%' OR sumber like '%$keyword%'  OR sudah_dihubungi like '%$keyword%') ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                                }
                            }else{
                                if(empty($keyword)){
                                    $query = mysqli_query($Conn, "SELECT*FROM kontak WHERE id_mitra='$SessionIdAkses' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                                }else{
                                    $query = mysqli_query($Conn, "SELECT*FROM kontak WHERE (id_mitra='$SessionIdAkses') AND ($keyword_by like '%$keyword%') ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                                }
                            }
