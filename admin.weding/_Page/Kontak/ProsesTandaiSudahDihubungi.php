<?php
    ...
    if(empty($_POST['check_id_kontak'])){
        ...
        if (isset($_POST['check_id_kontak'])) {
            ...
            // Membuat query untuk update data berdasarkan ID
            $idsToDelete = implode(',', $checkedKontak); 
            // Menggabungkan ID kontak menjadi string
            $query = "UPDATE kontak SET sudah_dihubungi='Sudah' WHERE id_kontak IN ($idsToDelete)";
            if (mysqli_query($Conn, $query)) {
                //Apabila Ada File Foto Maka Di Hapus
                $SimpanLog=addLog($Conn,$SessionIdAkses,$now,'Kontak','Update Kontak Sudah Dihubungi');
                if($SimpanLog=="Success"){
                    echo '<small class="text-success" id="NotifikasiTandaiDihubungiBerhasil">Success</small>';
                }else{
