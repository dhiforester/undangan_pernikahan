<?php
    ...
    if(empty($SessionIdAkses)){
        ...
        if(empty($_POST['id_supervisi'])){
            echo '<code class="text-danger">ID Supervisi tidak boleh kosong</code>';
        }else{
            if(empty($_POST['nama'])){
                ...
                if(empty($_POST['kontak'])){
                    ...
                    if(empty($_POST['email'])){
                        ...
                        if(empty($_POST['password'])){
                            ...
                            //Buka Data Lama
                            $FotoLama=GetDetailData($Conn,'supervisi','id_supervisi',$id_supervisi,'foto');
                            $KontakLama=GetDetailData($Conn,'supervisi','id_supervisi',$id_supervisi,'kontak');
                            $EmailLama=GetDetailData($Conn,'supervisi','id_supervisi',$id_supervisi,'email');
                            //Validasi Duplikat
                                ...
                                if(!empty($ValidasiKontakDuplikat)){
                                    ...
                                    if($JumlahKarakterKontak>20||!preg_match("/^[^a-zA-Z ]*$/", $_POST['kontak'])){
                                            ...
                                            if($tipe_gambar == "image/jpeg"||$tipe_gambar == "image/jpg"||$tipe_gambar == "image/gif"||$tipe_gambar == "image/png"){
                                                if($ukuran_gambar<2000000){
                                                    if(move_uploaded_file($tmp_gambar, $path)){
                                                        $FileLama = '../../assets/img/User/'.$FotoLama.'';
                                                        if (file_exists($FileLama)) {
                                                            if (unlink($FileLama)) {
                                                                $ValidasiGambar="Valid";
                                                            } else {
                                                                $ValidasiGambar="Terjadi kesalahan pada saat menghapus foto lama";
                                                            }
                                                        }else{
                                                            $ValidasiGambar="Valid";
                                                        }
                                        ...
                                        if($ValidasiGambar!=="Valid"){
                                            ...
                                            $Update = mysqli_query($Conn,"UPDATE supervisi SET 
                                                nama='$nama',
                                                kontak='$kontak',
                                                email='$email',
                                                password='$password',
                                                foto='$namabaru'
                                            WHERE id_supervisi='$id_supervisi'") or die(mysqli_error($Conn)); 
                                            if($Update){
                                                $KategoriLog="Supervisi";
                                                $KeteranganLog="Edit Supervisi baru";
